// apps/api/src/routes/assignment.js — per DevSpec §assignment routes
const router             = require('express').Router();
const pool               = require('../config/db');
const { requireRole }    = require('../middleware/roleCheck');
const { AppError }       = require('../utils/errors');

// POST /api/assignment/assign — 1 driver → 1 vehicle constraint enforced
router.post(
  '/assign',
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (!driver_id || !vehicle_id)
        throw new AppError('driver_id and vehicle_id are required', 400, 'VALIDATION_ERROR');
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      // Check for active incentive rule
