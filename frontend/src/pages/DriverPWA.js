...
export default function DriverPWA() {
  ...
  const [telemetry, setTelemetry] = useState({
    activeVehicle: "MH-12-QX-4019",
    battery: 92,
    driven: 45
  });
  const [recentPayments, setRecentPayments] = useState([
    { id: "TXN-001", amount: 850, date: "23-05-2026", type: "Daily Rent", isCredit: false, status: "Success", ref: "REF001" },
    { id: "TXN-002", amount: 1500, date: "22-05-2026", type: "Advance", isCredit: true, status: "Success", ref: "REF002" }
  ]);
  const [notifications, setNotifications] = useState([
    { id: 1, title: "System Alert", message: "Battery low", time: "11:30 AM", read: false }
  ]);
  ...
  const sendChatMessage = () => {
    ...
    setTimeout(() => {
      setChatHistory(prev => [...prev, { type: 'bot', message: 'Thank you! Support will respond shortly.' }]);
    }, 500);
    setChatMessage('');
  };
    ...
    <div className="flex flex-col gap-4 pb-4">
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        ...
        <div className="grid grid-cols-2 divide-x">
          <div className="p-3 text-center"><div className="text-lg font-mono font-bold">{telemetry.activeVehicle}</div><div className="text-[9px] text-slate-400">Registration</div></div>
          <div className="p-3 text-center"><div className="text-lg font-mono font-bold text-emerald-600">{telemetry.battery}%</div><div className="text-[9px] text-slate-400">Battery</div></div>
        </div>
        <div className="grid grid-cols-2 divide-x border-t">
          <div className="p-3 text-center"><div className="text-lg font-mono font-bold">{telemetry.driven} KM</div><div className="text-[9px] text-slate-400">Today's Distance</div></div>
  ...
  const AccountContent = () => (
    <div className="space-y-4 pb-4">
      <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-2xl p-5 text-white text-center">
        <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center text-xl font-bold mx-auto">{user?.name?.charAt(0) || 'R'}</div>
        <h2 className="text-lg font-bold mt-2">{user?.name || 'Rajesh Kumar'}</h2>
        <p className="text-blue-200 text-xs">{user?.usercode || 'DRV_DEMO_001'}</p>
      </div>
      <div className="bg-white border rounded-2xl p-4"><h3 className="font-bold mb-3 flex items-center gap-2"><User size={14} /> Personal Info</h3><div className="space-y-3"><div className="flex justify-between py-2 border-b"><span className="text-slate-500 text-xs">Phone</span><span className="font-medium text-xs">{user?.phone || '+91 9876542345'}</span><button onClick={() => navigator.clipboard.writeText(user?.phone || '9876542345')} className="text-blue-600"><Copy size={12} /></button></div><div className="flex justify-between py-2 border-b"><span className="text-slate-500 text-xs">Email</span><span className="font-medium text-xs">{user?.email || 'driver@mobilitygrid.com'}</span></div></div></div>
      <div className="bg-white border rounded-2xl p-4"><h3 className="font-bold mb-3 flex items-center gap-2"><Truck size={14} /> Assigned Vehicle</h3><div className="space-y-2"><div className="flex justify-between"><span className="text-slate-500 text-xs">Model</span><span className="font-medium text-xs">Tata Ace EV</span></div><div className="flex justify-between"><span className="text-slate-500 text-xs">Registration</span><span className="font-medium text-xs">MH-12-QX-4019</span></div></div></div>
      <div className="bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl p-4 text-white"><p className="text-xs opacity-80">Referral Code</p><p className="text-lg font-bold font-mono">{user?.usercode || 'DRV_DEMO_001'}</p><button onClick={() => navigator.clipboard.writeText(user?.usercode || 'DRV_DEMO_001')} className="mt-2 bg-white/20 px-3 py-1 rounded-lg text-xs flex items-center gap-1"><Copy size={12} /> Copy</button></div>
    </div>
  );
