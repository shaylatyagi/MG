  ...
  const HomeTab = () => (
    <div className="space-y-3 pb-4">
      ...
      {dues <= 0 ? (
        ...
        <div style={{background:'linear-gradient(135deg,var(--color-emerald-600) 0%,var(--color-emerald-500) 100%)',borderRadius:20,padding:20,display:'flex',alignItems:'center',gap:16}}>
        ...
        <div style={{background:'linear-gradient(135deg,var(--color-amber-700) 0%,var(--color-accent) 100%)',borderRadius:20,padding:20}}>
        ...
        <div style={{background:'linear-gradient(135deg,var(--color-primary) 0%,var(--color-violet) 100%)',borderRadius:20,padding:20}}>
          ...
          <button onClick={pay}
            style={{width:'100%',background:'var(--color-surface)',color:'var(--color-primary)',fontWeight:900,padding:'14px',borderRadius:14,fontSize:14,display:'flex',alignItems:'center',justifyContent:'center',gap:8,border:'none',cursor:'pointer',boxShadow:'0 4px 14px rgba(0,0,0,0.2)',transition:'transform 0.15s cubic-bezier(0.34,1.56,0.64,1)'}}
  ...
  const WalletTab = () => {
    ...
    return (
      <div className="space-y-3 pb-4 tab-fade">
        ...
        {/* Balance card */}
        <div style={{background:'linear-gradient(135deg,var(--color-gray-900) 0%,var(--color-gray-800) 100%)',borderRadius:20,padding:20}}>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div style={{background:'linear-gradient(135deg,var(--color-emerald-50),var(--color-emerald-100))',borderRadius:14,padding:'14px 12px',border:'1px solid var(--color-emerald-100)'}}>
              <p style={{fontSize:9,fontWeight:700,color:'var(--color-success-dark)',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:6}}>✓ {t.totalPaidLabel}</p>
              <p style={{fontSize:20,fontWeight:900,color:'var(--color-success-dark)',fontFamily:'monospace'}}>₹{totalPaid.toLocaleString('en-IN')}</p>
            </div>
            <div style={{background:dues > 0 ? 'linear-gradient(135deg,var(--color-accent-50),var(--color-amber-100))' : 'linear-gradient(135deg,var(--color-gray-50),var(--color-gray-100))',borderRadius:14,padding:'14px 12px',border:`1px solid ${dues > 0 ? 'var(--color-accent-50)' : 'var(--color-border)'}`}}>
              <p style={{fontSize:9,fontWeight:700,color:dues > 0 ? 'var(--color-accent-dark)':'var(--color-text-muted)',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:6}}>{dues > 0 ? '⏳' : '—'} {t.outstandingLabel}</p>
              <p style={{fontSize:20,fontWeight:900,color:dues > 0 ? 'var(--color-accent-dark)':'var(--color-text-secondary)',fontFamily:'monospace'}}>₹{dues.toLocaleString('en-IN')}</p>
            </div>
          </div>
        </div>
            ...
            {payments.length === 0 ? (
              ...
              <div key={i} onClick={() => { setSelTxn(p); setShowReceipt(true); }}
                ...
                <div style={{
                  ...
                  background: p.status === 'SUCCESS' ? 'var(--color-success-50)' : p.status === 'PENDING' ? 'var(--color-accent-50)' : 'var(--color-danger-50)'
                }}>
                  {p.status === 'SUCCESS'
                    ? <svg width={16} height={16} fill="none" stroke="var(--color-emerald-600)" strokeWidth={2.5} viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
                    : p.status === 'PENDING'
                    ? <svg width={16} height={16} fill="none" stroke="var(--color-amber-700)" strokeWidth={2.5} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                    : <svg width={16} height={16} fill="none" stroke="var(--color-danger-dark)" strokeWidth={2.5} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
                  }
                </div>
                {/* Text */}
                <div style={{flex:1,minWidth:0}}>
                  <p style={{fontSize:13,fontWeight:700,color:'var(--color-text)'}}>{p.type}</p>
                  <p style={{fontSize:10,color:'var(--color-text-muted)',fontFamily:'monospace',marginTop:2}}>{p.date}</p>
                </div>
                {/* Amount + status */}
                <div style={{textAlign:'right',flexShrink:0}}>
                  <p style={{fontSize:15,fontWeight:800,color: p.status === 'SUCCESS' ? 'var(--color-success-dark)' : p.status === 'PENDING' ? 'var(--color-accent-dark)' : 'var(--color-danger-dark)',fontFamily:'monospace'}}>
                  ...
                  <span style={{
                    ...
                    background: p.status === 'SUCCESS' ? 'var(--color-success-50)' : p.status === 'PENDING' ? 'var(--color-accent-50)' : 'var(--color-danger-50)',
                    color: p.status === 'SUCCESS' ? 'var(--color-success-dark)' : p.status === 'PENDING' ? 'var(--color-accent-dark)' : 'var(--color-danger-dark)',
                  }}>
    ...
    <div className="h-screen w-screen bg-slate-100 flex items-center justify-center overflow-hidden">
      <div className="w-full max-w-[412px] bg-slate-50 h-full sm:max-h-[840px] flex flex-col relative overflow-hidden sm:rounded-[32px] sm:shadow-2xl sm:border sm:border-slate-200">
        ...
        {paymentSuccess && (
          <div className="absolute inset-0 z-[110] flex flex-col items-center justify-center px-8"
            style={{background:'linear-gradient(160deg,var(--color-emerald-600) 0%,var(--color-teal-600) 50%,var(--color-gray-900) 100%)'}}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button onClick={requestNotifPermission} style={{
              ...
              background: 'var(--color-success-dark)', color: 'var(--color-text-inverse)', border: 'none',
      ...
      {showAddFunds && (
        <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.55)',zIndex:300,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div style={{background:'var(--color-surface)',borderRadius:'20px 20px 0 0',padding:'28px 20px 40px',width:'100%',maxWidth:480,boxShadow:'0 -8px 32px rgba(0,0,0,0.18)'}}>
            <h3 style={{fontSize:17,fontWeight:900,color:'var(--color-text)',marginBottom:4}}>Add Funds to Wallet</h3>
            <p style={{fontSize:12,color:'var(--color-text-secondary)',marginBottom:20}}>Enter the amount you want to add</p>
            <div style={{position:'relative',marginBottom:16}}>
              <span style={{position:'absolute',left:14,top:'50%',transform:'translateY(-50%)',fontSize:18,color:'var(--color-text-secondary)',fontWeight:700}}>&#8377;</span>
              <input
                type="number"
                inputMode="numeric"
                placeholder="0"
                ...
                autoFocus
                style={{width:'100%',boxSizing:'border-box',paddingLeft:34,paddingRight:14,paddingTop:14,paddingBottom:14,fontSize:24,fontWeight:900,color:'var(--color-text)',border:'2px solid var(--color-gray-200)',borderRadius:14,outline:'none',fontFamily:'inherit'}}
            ...
            <div style={{display:'flex',gap:8,marginBottom:12}}>
              {[100,500,1000].map(amt => (
                <button key={amt} onClick={() => setAddFundsAmt(String(amt))}
                  style={{flex:1,padding:'8px 0',background:'var(--color-gray-100)',border:'none',borderRadius:10,fontSize:13,fontWeight:700,color:'var(--color-gray-700)',cursor:'pointer'}}>
                  +&#8377;{amt}
                </button>
              ))}
            </div>
            <LoadingButton
              ...
              style={{width:'100%',padding:'14px',background:(!addFundsAmt||isNaN(addFundsAmt)||Number(addFundsAmt)<=0)?'var(--color-primary-light)':'var(--color-primary)',color:'var(--color-text-inverse)',border:'none',borderRadius:14,fontSize:15,fontWeight:800,cursor:(!addFundsAmt||isNaN(addFundsAmt)||Number(addFundsAmt)<=0)?'not-allowed':'pointer',marginBottom:10,fontFamily:'inherit'}}>
              Proceed to Pay
            </LoadingButton>
            <button onClick={() => setShowAddFunds(false)}
              style={{width:'100%',padding:'12px',background:'transparent',color:'var(--color-text-muted)',border:'none',fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>
              Cancel
            </button>
          </div>
        </div>
