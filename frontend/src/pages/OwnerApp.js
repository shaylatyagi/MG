...
const fetchAllData = useCallback(async () => {
  ...
  setDrivers(driversList);
    ...
    // Fetch real owner profile + company name
    try {
      const meRes = await fetch(`${API}/api/owner/me`, {
        headers: { Authorization: `Bearer ${token()}` }
      });
      const meData = await meRes.json();
      if (meData.success && meData.owner) setOwner(meData.owner);
  ...
  const getHeaderSubtitle = () => {
    const subtitles = {
      'home': 'Command Center',
      'drivers': `${stats.totalDrivers || filteredDrivers.length} Active Driver${stats.totalDrivers !== 1 ? 's' : ''}`,
      'vehicles': `${vehicles.length} Vehicles`,
      'payments': 'Transaction History',
      'profile': owner?.full_name || owner?.name || 'Owner Portal'
    };
