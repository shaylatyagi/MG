        ...
        createElement('div', { style: { marginTop: '20px' } }, [
          ...
          createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '8px' } }, 
            ['Vehicle RC', 'Insurance Certificate', 'Pollution Certificate'].map((doc) => 
              createElement('div', { key: doc, style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: '8px', border: '1px solid #E8E0D5', backgroundColor: '#FAF7F2' } }, [
                createElement('div', null, [
                  createElement('p', { style: { fontSize: '13px', color: '#1A1A1A', fontWeight: '500', margin: 0 } }, doc),
                  uploadedDocs[doc] && createElement('p', { style: { fontSize: '11px', color: '#16A34A', marginTop: '2px', margin: 0 } }, `✓ ${uploadedDocs[doc]}`)
                ]),
                createElement('label', { style: { backgroundColor: uploadedDocs[doc] ? '#DCFCE7' : '#8B5E3C', color: uploadedDocs[doc] ? '#16A34A' : 'white', padding: '6px 14px', borderRadius: '6px', fontSize: '12px', fontWeight: '600', cursor: 'pointer' } }, [
                  uploadedDocs[doc] ? 'Uploaded ✓' : 'Upload',
                  createElement('input', { type: 'file', style: { display: 'none' }, accept: '.pdf,.jpg,.jpeg,.png', onChange: (e) => handleFileUpload(doc, e.target.files[0]) })
                ])
              ])
            )
          )
    ...
    showEditModal && selectedVehicle && createElement('div', { style: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' } }, 
      createElement('div', { style: { backgroundColor: 'white', borderRadius: '16px', padding: '32px', width: '400px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' } }, [
        ...
        createElement('div', { style: { marginBottom: '24px' } }, [
          createElement('p', { style: labelStyle }, 'Driver'), 
